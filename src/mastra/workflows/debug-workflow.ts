...
import z from "zod";
import { gmailSearchEmails, getEmailContent, sendThreadReplyEmail } from "../../utils/gmail";
...
const searchTestEmails = createStep({
  ...
  description: "Searches for emails with subject 'Test: testing template formatting and naming issues!' and extracts basic information",
  inputSchema: z.boolean().describe("Boolean trigger to start the debug workflow"),
  outputSchema: z.array(
    z.object({
      emailId: z.string().describe("Gmail email ID"),
      messageId: z.string().nullable().describe("Message ID from email headers"),
  ...
  execute: async ({ inputData }) => {
    ...
    try {
      ...
      console.log(`📧 Found ${searchResult.length} emails matching the search criteria`);
...
const sendTestReplyEmails = createStep({
  ...
  description: "Sends thread reply emails using the extracted email data and sendThreadReplyEmail function",
  inputSchema: z.array(
    z.object({
      emailId: z.string().describe("Gmail email ID"),
      messageId: z.string().nullable().describe("Message ID from email headers"),
  ...
  outputSchema: z.object({
    ...
    successfulReplies: z.number().describe("Number of successful replies sent"),
    failedReplies: z.number().describe("Number of failed replies"),
    replyResults: z.array(
      z.object({
        ...
        replyMessageId: z.string().nullable().describe("ID of the sent reply message"),
        error: z.string().nullable().describe("Error message if failed"),
        senderName: z.string().nullable().describe("Name of original sender"),
        senderEmail: z.string().nullable().describe("Email of original sender")
      })
  ...
  execute: async ({ inputData }) => {
    ...
    console.log(`📤 Sending reply emails to ${inputData.length} test emails...`);
    ...
    for (const emailInfo of inputData) {
      try {
        ...
        if (!emailInfo.threadId || !emailInfo.messageId || !emailInfo.senderEmail) {
          console.warn(`⚠️ Missing required fields for email ${emailInfo.emailId}`);
          replyResults.push({
            emailId: emailInfo.emailId,
            threadId: emailInfo.threadId,
            success: false,
            replyMessageId: null,
            error: "Missing required fields (threadId, messageId, or senderEmail)",
        ...
        const replyParams = {
          ...
          templateId: "templates-request_key_details-resend_key_details", // Using a default template
         };
        ...
        if (replyResponse && replyResponse.id) {
          ...
          console.warn(`   ⚠️ Reply response missing ID for email ${emailInfo.emailId}`);
          replyResults.push({
            emailId: emailInfo.emailId,
            threadId: emailInfo.threadId,
            success: false,
        ...
        console.error(`❌ Error sending reply to email ${emailInfo.emailId}:`, error);
        replyResults.push({
          emailId: emailInfo.emailId,
          threadId: emailInfo.threadId,
          success: false,
    ...
    console.log(`   📈 Success rate: ${result.totalEmails > 0 ? Math.round((successfulReplies / result.totalEmails) * 100) : 0}%`);
